...
### 日次全体の型ミックス制約 (必須の品質ゲート)

**summary-card は 1 日の全図解の 60% 以下に抑える**こと。実測で summary-card 比率が 57%→81% (2026-06-30 は 100%) まで悪化した経緯があり、summary-card 偏重はページ全体が単調なテキスト箱の羅列になって「図解」の価値を失う。

- 全 items の figure を書き終える**前に型の割り当てを俯瞰**し、summary-card が 60% を超えそうなら、数値・対比・時系列を持つ記事を視覚 3 型 (comparison / metric-bars / timeline) へ振り直す
- 目安: 1 日 20 件なら **comparison + metric-bars + timeline を合計 8 件以上**。各型 1 件以上が理想
- **「summary-card から書き始めない」**: 各記事につき、まず 3 視覚型それぞれに当てはまらないかを確認し、全部外れた場合のみ summary-card にする (型選択フローの順守)
- 数値が 2 つ以上ある記事 → metric-bars か comparison が書けるはず。日付・バージョンが 2 つ以上 → timeline が書けるはず
- ただし**創作は禁止**: 記事に視覚化できる材料が本当に無い日は summary-card が多くても構わない (無理に数字をでっち上げる方が重罪)。その場合も各 summary-card の label/context/impact を厚くして質を担保する
...
### points は必ずオブジェクト（`{label, value}`）で書く

各 point は**必ず `label`（観点名 2-12字）と `value`（要点・値 1-36字）を持つオブジェクト**にする。
プレーンな文字列（`"○○が××した"` のような 1 行）を直接 `points` に入れてはいけない。
文字列だけだと `label` が空になり、図解の各行が「観点＋値」の二段組にならず、締まりのない箇条書きになる。

- ✅ Good: `{ "label": "対象", "value": "全AIチャットボット", "description": "ChatGPT/Claude等すべて" }`
- ❌ Bad: `"AIチャットボットへの健康情報開示内容も売却禁止対象"`（文字列直書き。label が無い）

`label` に入れる観点名の例: 「対象」「効果」「時期」「規模」「提供元」「価格」「反響」など。
記事から `label`/`value` に切り分けられない箇条書きは、その項目自体を `description` の厚い point 1 件に統合するか、点数を減らす。
