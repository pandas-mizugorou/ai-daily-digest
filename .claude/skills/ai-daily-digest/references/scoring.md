...
> **グラウンディング留意（Step 8.6 連動）**: 記事に裏付けの無い数値・主張で「深そう」に見えても depth は上げない。Step 8.6 で多くの修正・削除が必要になった記事（`grounding.flags` が大きい）は一次情報が薄い兆候であり、同点なら de-select 候補とする。
...
**設計方針（重要）**: タイトルのキーワード一致だけでは大半が `summary-card` に落ち、図が「ただのテキスト」になって一目で理解できない。判定は **title + summary_en + raw_excerpt 全体の構造**（数値・対比語・日付の出現）を見て、視覚 3 型（comparison / metric-bars / timeline）を**積極的に拾い上げる**。`summary-card` は構造が本当に取れない記事だけの最後の受け皿とする。

```python
import re

def hint_figure_type(item):
    # タイトルだけでなく本文抜粋まで含めて構造を見る（キーワード一致依存からの脱却）
    text = " ".join(filter(None, [item.title, item.summary_en, item.raw_excerpt])).lower()

    # --- 構造シグナルを抽出 ---
    # 数値トークン（スコア・%・容量・価格・倍率など）。3 個以上 → 棒グラフ向き
    numbers = re.findall(r"\d[\d,.]*\s*(?:%|pt|pts|x|倍|k|m|b|ms|tok|gb|tb|\$)?", text)
    num_count = len(numbers)

    # 日付・時点トークン（年・四半期・月）。3 個以上 → 時系列向き
    dates = re.findall(r"(?:20\d\d[/年.\-]\d{1,2}|20\d\d|q[1-4]\s*20\d\d|[1-9]\d?月|h[12]\s*20\d\d)", text)
    date_count = len(set(dates))

    benchmark_kw = ["mmlu", "gsm8k", "humaneval", "swe-bench", "gpqa", "mmmu", "benchmark", "score", "ベンチマーク", "精度", "正答率"]
    comparison_kw = [" vs ", " vs. ", "versus", "before", "after", "比較", "対比", "→", "->", "から", "upgrade", "アップグレード",
                     "前世代", "従来", "改善", "向上", "outperform", "compared to", "previous", "predecessor", "v1", "v2"]
    version_bump = re.search(r"\b\d+(\.\d+)?\s*(?:→|->|to|から)\s*\d+(\.\d+)?\b", text)  # 例: 4.5 → 5
    timeline_kw = ["roadmap", "timeline", "schedule", "rollout", "rolling out", "launch plan", "ga ", "preview", "coming",
                   "ロードマップ", "段階", "ステップ", "順次", "公開予定", "発表予定", "計画"]

    # --- 判定（視覚 3 型を優先。複数該当時は記事に最も具体性が乗る型を選ぶ）---

    # 1. 対比構造（旧 vs 新 / A 社 vs B 社 / バージョン更新）→ comparison
    #    新旧モデル更新は AI ニュースで最頻出。バージョン番号の遷移や比較語があれば拾う。
    if version_bump or any(c in text for c in comparison_kw):
        return "comparison"

    # 2. 数値指標が 3 個以上 / ベンチマーク言及 → metric-bars
    if (any(b in text for b in benchmark_kw) and num_count >= 2) or num_count >= 3:
        return "metric-bars"

    # 3. 時系列（ロードマップ語 or 異なる日付が 3 個以上）→ timeline
    if any(t in text for t in timeline_kw) or date_count >= 3:
        return "timeline"

    # 4. 構造が取れない場合のみ summary-card（最後の受け皿）
    return "summary-card"
```

**注意**: このヒントは「視覚型を広めに提案する」設計で、誤検出（実際は summary-card が適切）は LLM 側が記事内容を見て補正する前提。逆方向の取りこぼし（視覚化できる記事を summary-card にしてしまう）の方が品質への悪影響が大きいため、視覚 3 型に倒す。
