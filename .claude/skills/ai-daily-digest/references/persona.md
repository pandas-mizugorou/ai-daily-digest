...
- **事実厳格性（グラウンディング）**: `summary_ja` / `key_points_ja` / `figure` / `x_post` の数値・固有名詞・日付は **記事内の事実のみ**。Step 8.6 のグラウンディング検証（SKILL.md）を必ず通し、裏付けの無い主張は書き直す・落とす・図を summary-card に降格する。迷ったら推測せず落とす。Top Picks の `x_post` は X 自動投稿されるため特に厳格に扱う
